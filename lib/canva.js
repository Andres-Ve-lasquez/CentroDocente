...
const DEFAULT_SCOPES = ["design:meta:read"];
...
function getCanvaScopes() {
  return process.env.CANVA_SCOPES?.split(/[,\s]+/).filter(Boolean) || DEFAULT_SCOPES;
}

export function buildCanvaAuthorizationUrl({ challenge, state, scopes = getCanvaScopes() }) {
